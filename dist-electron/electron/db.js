...
const dbFolder = electron_1.app.getPath('userData');
const dbPath = node_path_1.default.join(dbFolder, 'aquadistrib.sqlite');
async function ensureDB() {
    if (!node_fs_1.default.existsSync(dbFolder))
        node_fs_1.default.mkdirSync(dbFolder, { recursive: true });
...
function seedIfEmpty() {
    const count = exports.db.prepare('SELECT COUNT(*) as c FROM companies').get();
    if (count.c > 0)
        return;
    const insertCompany = exports.db.prepare(`
  ...
  `);
    const insertPartner = exports.db.prepare(`
  ...
  `);
    const insertKanban = exports.db.prepare(`
